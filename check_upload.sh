...
cd Cython
CYTHON_VERSION=`git describe --tags`
if [ "${CYTHON_VERSION:0:1}" != 'v' ]; then
    echo "Cython version $CYTHON_VERSION does not start with 'v'"
    exit 1
fi
WHEEL_HEAD="Cython-${Cython_VERSION:1}"
